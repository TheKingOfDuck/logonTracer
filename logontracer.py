...
def parse_evtx(evtx_list):
    ...
    for ipaddress in event_set["ipaddress"].drop_duplicates():
        if ipaddress in hosts_inv:
            ...
            hostname = ipaddress
        tx.run(statement_ip, {"IP": ipaddress, "rank": ranks[ipaddress], "hostname": hostname})
    ...
    for username in username_set:
        ...
        if not ustatus:
            ustatus = "-"
        tx.run(statement_user, {"user": username[:-1], "rank": ranks[username], "rights": rights, "sid": sid, "status": ustatus,
    ...
    for domain in domains:
        tx.run(statement_domain, {"domain": domain})

    for _, events in event_set.iterrows():
        tx.run(statement_r, {"user": events["username"][:-1], "IP": events["ipaddress"], "id": events["eventid"], "logintype": events["logintype"],
                                "status": events["status"], "count": events["count"], "authname": events["authname"]})

    for username, domain in domain_set_uniq:
        tx.run(statement_dr, {"user": username[:-1], "domain": domain})

    tx.run(statement_date, {"Daterange": "Daterange", "start": datetime.datetime(*starttime.timetuple()[:4]).strftime("%Y-%m-%d %H:%M:%S"),
                               "end": datetime.datetime(*endtime.timetuple()[:4]).strftime("%Y-%m-%d %H:%M:%S")})

    if len(deletelog):
        tx.run(statement_del, {"deletetime": deletelog[0], "user": deletelog[1], "domain": deletelog[2]})
    ...
    if len(policylist):
        ...
        for policy in policylist:
            ...
            if policy[3] in AUDITING_CONSTANTS:
                ...
                sub = policy[3]
            username = policy[1]
            tx.run(statement_pl, {"id": id, "changetime": policy[0], "category": category, "sub": sub})
            tx.run(statement_pr, {"user": username[:-1], "id": id})
            id += 1
